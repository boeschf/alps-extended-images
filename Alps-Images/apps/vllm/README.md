...
The CUDA image also installs Run:ai Model Streamer `0.16.1`. NIXL is intentionally omitted: this image has no validated prefill/decode-disaggregation deployment yet, and adding a second data plane without that contract would leave an untested runtime path.

Both builds constrain Python package installation to the Torch and Triton versions supplied by their canonical base images; the CUDA build also preserves base-provided TorchVision and TorchAudio versions. A final image check rejects dependency resolution that replaces any of those packages. Runtime Python ignores user-site packages (`PYTHONNOUSERSITE=1`) and keeps the CSCS PyPI proxy as its default package index. Image labels record the pinned UCCL revision and, where applicable, the AITER revision and Model Streamer version.

The CUDA source build uses vLLM's upstream conservative defaults of two build jobs and eight NVCC threads. This reduces peak compiler concurrency from the previous 32-job configuration while retaining explicit build-argument overrides for controlled builders.
